 ...
 *
 *   · Tiempo. Unos 40 s por respuesta; siete estudiantes × cinco preguntas
 *     son ~20 minutos. No cabe en el aula.
 *   · Y la que importa: **nada generado por máquina se proyecta sin que el
 *     docente lo haya leído antes.** Este curso evalúa verificar lo que se
 *     cita; proyectar una paráfrasis automática de lo que escribió un
...
// ── Señales, las mismas que muestra el tablero ───────────────────────
// AUSENCIA: la persona reporta que no encontró el dato / no hay protocolo.
// Es el hallazgo más valioso del curso (P1 pide documentarlo).
const RE_AUS = /(?:no\s+(?:encontr\w+|hay|existe\w*|aparece\w*|figura\w*|se\s+(?:ofrece|registr|mide|documenta|hace)\w*|tenemos|contamos|pude\w*)|sin\s+(?:registro|datos?|protocolo\w*|informaci[óo]n|evidencia|programa\w*)|ausencia\s+de\s+\w+|no\s+(?:existe|se\s+conoce)|nada\s+formal)/i;
// FUENTE: marcadores de que la respuesta se apoya en literatura o en un
// instrumento de medición. Contexto de rehabilitación e investigación.
const RE_FUE = /(?:seg[úu]n\s+\w+|p[áa]g(?:ina)?\.?\s*\d+|doi|https?:\/\/\S+|OMS|WHO|IAHPC|OPS|Cochrane|PubMed|SINAB|Tiberini|Minosso|Santiago-?Palma|Lunney|nivel\s+de\s+evidencia|ensayo\s+(?:cl[íi]nico|aleatoriz\w+)|revisi[óo]n\s+(?:sistem\w+|integr\w+)|meta[- ]?an[áa]lisis|escala|[íi]ndice\s+de\s+barthel|cambio\s+m[íi]nimo|MCID|FIM|PROM)/i;
...
(async function () {
  ...
  for (const q of datos.preguntas) {
    ...
    out.push(`**Señales** · ${conTexto.length}/${datos.inscritos} respondieron · ` +
      `**${aus}** reportan que no hay dato / protocolo · ${fue} se apoyan en fuente o instrumento · ` +
