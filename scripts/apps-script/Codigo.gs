/**
 * ============================================================================
 * MOTOR DE INVESTIGACIÓN EN CP Y REHABILITACIÓN — Google Apps Script
 * Investigación en Cuidados Paliativos y Rehabilitación · 37543013 · UAN
 * ----------------------------------------------------------------------------
 ...
 * cómoda, pero aquí el entregable abre días antes de la sesión: la lista
 * quedaría expuesta todo ese tiempo. Un correo que no está en el roster
 * simplemente no entra, y eso basta.
 *
 * ----------------------------------------------------------------------------
 * SEUDÓNIMOS
 *
 * Cada estudiante tiene un seudónimo estable («A», «B», …) que se asigna en el
 * roster. El tablero proyecta el seudónimo, no el nombre: cuando se pide tomar
 * posición, el anonimato al proyectar sube la honestidad de lo que se escribe.
 * El docente ve la correspondencia en su propio panel. (Con 7 personas que
 * escriben cada una sobre su servicio, el anonimato baja la exposición pero no
 * la garantiza del todo; conviene decirlo así en el encuadre.)
 * ============================================================================
 */
...
function inicializar() {
  ...
  if (!config) {
    ...
    var sesiones = [
      ['S01', 'Presentación y Unidad 1: trayectorias, modelos y tecnologías', '', '', '', 'No'],
      ['S02', 'Rehabilitación paliativa en cáncer',                           '', '', '', 'No'],
      ['S03', 'Rehabilitación paliativa en falla de órgano',                  '', '', '', 'No'],
      ['S04', 'Trastornos neurodegenerativos (sesión en inglés)',            '', '', '', 'No'],
      ['S05', 'Rehabilitación paliativa en demencia y fragilidad',           '', '', '', 'No'],
      ['S06', 'Rehabilitación paliativa en eventos catastróficos',           '', '', '', 'No'],
      ['S07', 'Seminario integrador y socialización de productos',           '', '', '', 'No']
    ];
    ...
    for (var i = 0; i < sesiones.length; i++) {
      sesiones[i][2] = sesiones[i][0] + '_INV_REHAB_2026II_' + cadenaAleatoria_(6);
    }
  ...
  if (!props.getProperty('DASH_TOKEN')) {
    var t = 'DASH_INV_REHAB_2026II_' + cadenaAleatoria_(10);
 ...
 *
 * ⚠️ BARAJA ANTES DE ASIGNAR, y eso no es un adorno. El roster suele estar en
 * orden alfabético; si las letras se repartieran en orden de fila, «A» sería el
...
function doGet(e) {
  ...
  try {
    switch (p.action) {
      ...
      default:        return json_({ ok: true, servicio: 'Motor de Investigación en CP y Rehabilitación', version: '1.0' });
    }
  } catch (err) {
 ...
 * entregable se puede corregir hasta el cierre. Es deliberado — se premia que
 * se vuelva sobre lo escrito, no que se acierte a la primera.
 */
function doPost(e) {
